...
function math_captcha_settings_page(): void
{
    ...
    if (isset($_POST['math_captcha_save']) && isset($_POST['nonce']) && yourls_verify_nonce('math_captcha_save_settings', $_POST['nonce'])) {
    ...
    echo '<input type="hidden" name="nonce" value="' . yourls_esc_attr(yourls_create_nonce('math_captcha_save_settings')) . '" />';
    echo '<table class="form-table">';
    echo '<tr>';
    echo '<th scope="row"><label for="math_captcha_min">Minimum Number</label></th>';
    echo '<td>';
    echo '<input type="number" id="math_captcha_min" name="math_captcha_min" value="' . yourls_esc_attr((string) $min) . '" min="0" max="99" />';
    echo '<p class="description">The minimum number to use in addition problems (default: 1).</p>';
    echo '</td>';
    echo '</tr>';
    echo '<tr>';
    echo '<th scope="row"><label for="math_captcha_max">Maximum Number</label></th>';
    echo '<td>';
    echo '<input type="number" id="math_captcha_max" name="math_captcha_max" value="' . yourls_esc_attr((string) $max) . '" min="1" max="99" />';
    echo '<p class="description">The maximum number to use in addition problems (default: 49). Must be greater than or equal to the minimum.</p>';
    echo '</td>';
    echo '</tr>';
    echo '</table>';
    echo '<p class="submit"><input type="submit" name="math_captcha_save" class="button-primary" value="Save Settings" /></p>';
    echo '</form>';
    echo '</div>';
}
