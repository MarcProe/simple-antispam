...
- The plugin `Version:` header in `plugin.php` is asserted by the unit tests —
  bump both together.
- After opening a PR, always watch it: subscribe to its webhook activity and
  keep it green — autofix CI failures and address review comments — until it is
  merged or closed. This is the default for every PR, not something to ask about
  per-PR. Rely on the webhook subscription to deliver events; do not poll or
  sleep-wait on CI.
